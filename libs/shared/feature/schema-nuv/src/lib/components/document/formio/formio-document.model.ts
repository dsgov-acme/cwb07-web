...
import { FormioDocumentComponent } from './formio-document.component';
...
const type = AttributeTypes.DOCUMENT;
...
export function registerDocumentComponent(injector: Injector) {
  if (!customElements.get(selector)) {
    registerCustomFormioComponent(COMPONENT_OPTIONS, FormioDocumentComponent, injector);
  }
}
...
function editForm() {
  return {
    components: [
      {
        // fields
        components: [{ ...defaultPropertyNameConfiguration }, { ...defaultProcessorsConfiguration }, { ...defaultAntivirusMessage }],
      },
    ],
  };
}
