...
import { FormioStringComponent } from './formio-string.component';
...
const type = AttributeTypes.STRING;
...
export function registerStringComponent(injector: Injector) {
  if (!customElements.get(selector)) {
    registerCustomFormioComponent(COMPONENT_OPTIONS, FormioStringComponent, injector);
  }
}
