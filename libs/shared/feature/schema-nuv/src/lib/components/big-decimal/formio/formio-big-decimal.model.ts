...
import { FormioBigDecimalComponent } from './formio-big-decimal.component';
...
const type = AttributeTypes.BIGDECIMAL;
...
export function registerBigDecimalComponent(injector: Injector) {
  if (!customElements.get(selector)) {
    registerCustomFormioComponent(COMPONENT_OPTIONS, FormioBigDecimalComponent, injector);
  }
}
...
function editForm() {
  return {
    components: [
      {
        // fields
        components: [{ ...defaultPropertyNameConfiguration }],
      },
    ],
  };
}
