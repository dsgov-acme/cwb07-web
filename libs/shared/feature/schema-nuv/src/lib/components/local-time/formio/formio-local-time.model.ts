...
import { FormioLocalTimeComponent } from './formio-local-time.component';
...
const type = AttributeTypes.LOCALTIME;
...
export function registerLocalTimeComponent(injector: Injector) {
  if (!customElements.get(selector)) {
    registerCustomFormioComponent(COMPONENT_OPTIONS, FormioLocalTimeComponent, injector);
  }
}
